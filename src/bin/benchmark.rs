...
fn main() {
    ...
    let num_blocks = 500_000;
    let num_outputs = 100_000;

    println!("Generating {} blocks of data for performance test...", num_blocks);
    ...
    println!("Naive FP32 Duration: {:?}", naive_duration);
    ...
    println!("vec101 SIMD Duration: {:?}", simd_duration);

    let speedup = naive_duration.as_secs_f64() / simd_duration.as_secs_f64();
